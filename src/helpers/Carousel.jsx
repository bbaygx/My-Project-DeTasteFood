...
export default function Carousel() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            // request data dari API berdasarkan params region
            const data = await getByLocation(region);
            // filter data berdasarkan rating di atas 4
            const terlarisItems = data.data.filter(item => item.rating > 4);
            setTerlaris(terlarisItems);
        }

        fetchData();
        // update data ketika region berubah
    }, [region])
    ...
    // Mengacak data terlaris sebelum ditampilkan
    const shuffledTerlaris = shuffleArray(terlaris);
